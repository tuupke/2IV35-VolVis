 ...
 */
public class TreePlot extends Display {
     ...
     * 
     */
    public TreePlot(Table table, int NUMOFSETS) {
...
//            System.out.println(t.getColumnName(i));
        ...
        double[][] range = new double[colnames.length][2];

        for (int i = 0; i < colnames.length; ++i) {
            String name = (String) colnames[i].get("Label");
            range[i][0] = (Double) table.get(table.getMetadata(name).getMinimumRow(), name);
            range[i][1] = ((Double) table.get(table.getMetadata(name).getMaximumRow(), name) - range[i][0]) / NUMOFSETS;
        }
...
//                System.out.println("ans "+(int)(((Double)tuple.get((String) colnames[i].get("Label"))-range[i][0])/range[i][1]));
                int setNum = (int) Math.floor(((Double) tuple.get((String) colnames[i].get("Label")) - range[i][0]) / range[i][1]);
                if (setNum == NUMOFSETS) {
                    setNum--;
                }
            ...
            for (int q = 0; q < 11; q++) {
                if (values[i][q][NUMOFSETS] > 0) {
                    ...
                    for (int s = 0; s < NUMOFSETS; s++) {
                        ...
                        String lab;
                        if (s == NUMOFSETS - 1) {
                            lab = "[" + one.format(range[i][0] + s * range[i][1]) + " - " + one.format(range[i][0] + (s + 1) * range[i][1]) + "]";
                        } else {
                            lab = "[" + one.format(range[i][0] + s * range[i][1]) + " - " + one.format(range[i][0] + (s + 1) * range[i][1]) + ")";
                        }
...
//            graph.addEdge(colnames[i], n);
        ...
        int[] palette = new int[NUMOFSETS+2];
        palette[0]=ColorLib.rgb(200, 200, 200);
        palette[1]=ColorLib.rgb(0, 0, 255);
        for (int s = 0; s < NUMOFSETS; s++) {
            if( s <= NUMOFSETS/2){
                palette[s+2]=ColorLib.rgb(255, (int)(255*((s*2)/(NUMOFSETS-1.0))), 0);
            }else{
                palette[s+2]=ColorLib.rgb(512-(int)(255*((s*2)/(NUMOFSETS-1.0))),255, 0);
            }
        }
   
//                {
//            , , ColorLib.rgb(255, 0, 0), ColorLib.rgb(255, 255, 0), ColorLib.rgb(0, 255, 0)
//        };
